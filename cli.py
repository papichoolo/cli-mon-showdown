...
class GameWindow:
    ...
    def __init__(self, enabled: bool = True, feed_lines: int = 8):
        ...
        self.feed_lines = max(
            15, min(25, feed_lines)
        )  # Increased from 15 to 25 max lines
        self.feed: List[str] = []
        self.mounted = False
    ...
    def render(
        self,
        battle: Dict[str, Dict[str, Optional[object]]],
        title: str = "CLI-Mon Showdown",
    ):
        if not self.enabled:
            return
        ...
        def side_line(label: str, side: Dict[str, Optional[object]]) -> str:
            ...
            if fainted:
                ...
                bar = self._bar(
                    hp if isinstance(hp, int) else None,
        ...
        feed_header = (
            "│"
            + (feed_title + "─" * max(0, (w - 2 - len(" Battle Log ") - 1))).ljust(
                w - 2, "─"
            )
            + "│"
        )
        ...
        try:
            ...
            debug_print(
                f"ANSI rendering failed: {e}, falling back to simple print", "UI"
            )
            print(new_render)
...
def _update_battle_state_from_line(
    line: str, battle: Dict[str, BattleSide]
) -> Tuple[bool, bool]:
    ...
    if tag in ("switch", "drag") and len(parts) >= 3:
        ...
        if side in battle:
            ...
            debug_print(
                f"HP token parsed - HP: {hp}, MaxHP: {maxhp}, Fainted: {fainted}",
                "BATTLE_STATE",
            )
            if fainted:
                if battle[side].get("fainted") is not True:
...
def _process_output(
    ...
    shown_rqid: Dict[str, Optional[int]],
    battle: Dict[str, BattleSide],
    ai_error_count: Dict[str, int],
    current_turn: int = 0,
    ui: Optional[GameWindow] = None,
    ...
    for line in out_lines:
        ...
        if "|turn|" in line:
            ...
            debug_print(
                "New turn detected; reset shown_rqid for both sides", "REQUESTS"
            )
        # Check for player errors first to handle error recovery
        if "|error|" in line and (
            "Invalid choice" in line or "Unavailable choice" in line
        ):
            ...
            for side in shown_rqid:
                if side == active_side:
                    ...
                    debug_print(
                        f"Reset request ID for {side} to force re-prompt",
                        "ERROR_RECOVERY",
                    )
            # Add error message to UI
            error_msg = (
                line.split("|error|", 1)[1] if "|error|" in line else "Invalid choice"
            )
        ...
        try:
            ...
            debug_print(
                f"Error processing battle state line '{line.strip()}': {e}", "ERROR"
            )
        ...
        if ui and ui.enabled:
            ...
            if humanize:
                ...
                if (
                    clean_line
                    and clean_line not in seen_messages
...
def pack_team(
    path: str, formatname: str = "gen7ou", ps_path: str = "pokemon-showdown"
) -> str:
    import os
    ...
    try:
        ...
        debug_print(
            f"Node.js or pokemon-showdown not found when packing {path}: {e}",
            "TEAM_ERROR",
        )
    ...
    try:
        ...
        if validate_result.returncode != 0:
            ...
            if error_output:
                ...
                print("=" * 50)
                raise RuntimeError(
                    f"Team validation failed for {path} in format {formatname}:\n{error_output}"
                )
            else:
                ...
                print(
                    "The team may contain banned Pokemon, moves, or items for this format."
                )
                raise RuntimeError(
                    f"Team validation failed for {path} in format {formatname}"
                )
        ...
        debug_print(
            f"Node.js or pokemon-showdown not found when validating {path}: {e}",
            "TEAM_ERROR",
        )
        raise RuntimeError("Node.js not found or pokemon-showdown directory missing")
    ...
    debug_print(
        f"Team {path} packed and validated successfully for format {formatname}", "TEAM"
    )
    return packed_team
    ...
    for raw in lines:
        ...
        if "|request|" in line and current in ("p1", "p2"):
            try:
                ...
                debug_print(
                    f"Parsed request for {current}: {list(req.keys())}", "REQUESTS"
                )
            except (json.JSONDecodeError, IndexError) as e:
                # Log malformed JSON but keep last good request
                debug_print(
                    f"Malformed request JSON for {current}: {e}", "REQUESTS_ERROR"
                )
            except Exception as e:
                # Log unexpected parsing errors
                debug_print(
                    f"Unexpected error parsing request for {current}: {e}",
                    "REQUESTS_ERROR",
                )
...
def _create_agent_observation(
    ...
    for i, pokemon in enumerate(pokemon_list, start=1):
            ...
            if "/" in condition:
                ...
                if "/" in hp_part:
                    try:
                        ...
                        hp_info["hp_percent"] = int(
                            round((int(current_hp) / int(max_hp)) * 100)
                        )
                    except ValueError:
                        pass
        ...
        bench_data.append(
            {
                "index": i,
                "species": pokemon.get("details", "").split(",")[0]
                if pokemon.get("details")
                else "Unknown",
            ...
            available_switches.append(
                {
                    ...
                    "details": pokemon.get("details", ""),
                    "condition": pokemon.get("condition", ""),
                    "hp_status": pokemon.get("condition", "").split()[0]
                    if pokemon.get("condition")
                    else "",
                }
            )
...
def _llm_agent_decision(
    observation: dict, team_knowledge: Optional[dict] = None, raw_log: str = ""
) -> dict:
    """
    ...
    Args:
        ...
        raw_log: Raw showdown log for the current turn

    Returns:
        Dictionary with 'action_type' ('move' or 'switch') and 'choice' (index or move name)
    """
    debug_print(
        f"LLM agent making decision. Forced switch: {observation.get('is_forced_switch', False)}",
        "LLM_AGENT",
    )
    ...
    try:
        if GEMINI_AVAILABLE:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(get_gemini_decision, observation, team_knowledge, raw_log=raw_log)
                try:
                    # 60 second timeout for LLM response
                    decision = future.result(timeout=60.0)
                    return decision
                except concurrent.futures.TimeoutError:
                    debug_print("LLM agent timed out after 60 seconds", "LLM_AGENT")
        ...
        if available_moves:
            ...
            valid_moves = [m for m in available_moves if not m.get("disabled", False)]
            if not valid_moves:
                valid_moves = available_moves # fallback if all are marked disabled somehow
            ...
            if best_move:
                debug_print(
                    f"LLM agent chose move: {best_move['index']} ({best_move.get('move', 'unknown')})",
                    "LLM_AGENT",
                )
                return {
                    "action_type": "move",
        ...
        if available_switches:
            ...
            debug_print(
                f"LLM agent chose switch: {choice['index']} ({choice['species']})",
                "LLM_AGENT",
            )
            return {
                "action_type": "switch",
...
def _translate_agent_decision(decision: dict, ai_req: dict) -> Optional[str]:
    ...
    if action_type == "switch":
        ...
        if choice in valid_indices:
            ...
            debug_print(
                f"Invalid switch choice {choice}, valid options: {valid_indices}",
                "TRANSLATION_ERROR",
            )
        ...
        if choice in valid_indices:
            ...
            debug_print(
                f"Invalid move choice {choice}, valid options: {valid_indices}",
                "TRANSLATION_ERROR",
            )
    ...
    try:
        ...
        for line in lines:
            ...
            if (
                ...
                and not line.startswith("Ability:")
                and not line.startswith("EVs:")
                and not line.startswith("IVs:")
                and not line.startswith("Nature:")
                and "@" in line
            ):
                if current_pokemon:
        ...
        debug_print(
            f"Parsed team knowledge for {side}: {len(team_knowledge['pokemon'])} Pokemon",
            "TEAM_KNOWLEDGE",
        )
...
def _show_pokemon_showdown_menu(
    req: dict,
    battle: Dict[str, BattleSide],
    active_side: str,
    ui: Optional[GameWindow] = None,
    ...
    if ui and ui.enabled:
        ...
        sys.stdout.write(
            _ansi(f"{ui.feed_lines + 10}H")
        )  # Move to line well below window
        sys.stdout.flush()
        ...
        try:
            ...
            if main_choice == "1":  # Fight
                ...
                while True:
                    try:
                        move_choice = input(
                            f"\nSelect move (1-{len(moves)} or 'back'): "
                        ).strip()
                        ...
                        if 1 <= move_num <= len(moves):
                            ...
                            original_index = selected_move.get(
                                "original_index", move_num
                            )
                ...
                while True:
                    try:
                        switch_choice = input(
                            f"\nSelect Pokemon (1-{len(valid_switches)} or 'back'): "
                        ).strip()
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        help="Path to Player 1 team file (Showdown importable)",
    )
    parser.add_argument(
        "p2", nargs="?", default=None, help="Path to Player 2 team file"
    )
    parser.add_argument("--format", default="gen7ou")
    parser.add_argument(
        "--remote",
        action="store_true",
        help="Connect to the live Pokemon Showdown server",
    )
    parser.add_argument(
        "--username",
        default="",
        help="Username for the live Pokemon Showdown server",
    )
    parser.add_argument(
        "--password",
        default="",
        help="Password for the live Pokemon Showdown server",
    )
    parser.add_argument(
        "--randbat",
        action="store_true",
        help="Enable random battles, generating teams automatically.",
    )
    parser.add_argument(
        "--no-auto-preview",
        action="store_true",
    ...
    parser.add_argument(
        "--no-p2-ai", dest="p2_ai", action="store_false", help="Disable player 2 AI"
    )
    parser.add_argument(
        "--humanize",
        dest="humanize",
    ...
    if GEMINI_AVAILABLE and args.p2_ai:
        try:
            ...
            if api_key:
                ...
                debug_print(
                    "No API key found for Gemini. Set GOOGLE_AI_API_KEY environment variable to use AI agent",
                    "MAIN",
                )
        except Exception as e:
    ...
    # Validate arguments
    if not args.remote and not args.randbat and (not args.p1 or not args.p2):
        parser.error(
            "Error: p1 and p2 team file arguments are required unless --randbat or --remote is specified.\n"
    ...
    if not args.remote:
        try:
            if args.randbat:
                ...
                debug_print(
                    f"P1 team packed length: {len(p1_team)}, P2 team packed length: {len(p2_team)}",
                    "TEAMS",
                )
                ...
                debug_print(
                    f"Team knowledge parsed for P2: {len(team_knowledge.get('pokemon', []))} Pokemon",
                    "TEAMS",
                )
    ...
    try:
        if args.remote:
            if not args.username:
                print("Error: --username is required when using --remote")
                return
            from remote_showdown import RemoteShowdownWrapper
            sim = RemoteShowdownWrapper(args.username, args.password, args.format)
        else:
    ...
    # For remote games, waits can be long (matchmaking, opponent thinking).
    # 6000 loops * 0.1s = 10 minutes timeout
    max_ai_loops = 6000 if args.remote else 15  # Prevent infinite loops
    preview_done = set()
    ...
    try:
        ...
        while True:
            ...
            debug_print(
                f"Simulator output: {len(out) if out else 0} lines", "SIMULATOR"
            )
            if out:
                for line in out:
                ...
                debug_print(
                    f"Process output result - Winner: {winner}, Error: {player_error_detected}",
                    "MAIN",
                )
                if winner:
                ...
                if p1_fainted and p2_fainted:
                    ...
                    p1_can_switch = any(
                        not p.get("condition", "").endswith("fnt")
                        for p in p1_req.get("side", {}).get("pokemon", [])
                        if not p.get("active")
                    )
                    p2_can_switch = any(
                        not p.get("condition", "").endswith("fnt")
                        for p in p2_req.get("side", {}).get("pokemon", [])
                        if not p.get("active")
                    )
                    ...
                    if not p1_can_switch or not p2_can_switch:
                        debug_print(
                            "Double KO detected, but one player has no remaining Pokemon. Game should end.",
                            "MAIN",
                        )
                    else:
                        if p1_can_switch and p2_can_switch:
                            ...
                            try:
                                ...
                                if (
                                    human_req
                                    and (human_req.get("forceSwitch") or [False])[0]
                                ):
                                    ...
                                    if shown_rqid.get(human_side) != player_rqid:
                                        ...
                                        choice = _show_pokemon_showdown_menu(
                                            human_req, battle, human_side, ui
                                        )
                                ...
                                debug_print(
                                    f"Error handling double-KO switch prompt: {e}",
                                    "MAIN",
                                )
                ...
                if player_error_detected:
                    debug_print(
                        "Player error detected, waiting for updated requests",
                        "ERROR_RECOVERY",
                    )
                    ...
                    if additional_out:
                        debug_print(
                            f"Got additional output after error: {len(additional_out)} lines",
                            "ERROR_RECOVERY",
                        )
                ...
                for side in ("p1", "p2"):
                    ...
                    debug_print(
                        f"Side {side} request: {bool(req)}, has teamPreview: {bool(req and req.get('teamPreview'))}, preview_done: {side in preview_done}",
                        "PREVIEW",
                    )
                    if req and req.get("teamPreview") and side not in preview_done:
                        ...
                        if n:
                            ...
                            debug_print(
                                f"Default team order for {side}: {default_order}",
                                "PREVIEW",
                            )
                            ...
                            if ui and ui.enabled:
                                ...
                                print(note)
                            debug_print(
                                f"Auto team preview sent for {side}: {default_order}",
                                "PREVIEW",
                            )
                ...
                if args.p2_ai:
                    ...
                    debug_print(
                        f"AI loop counter incremented to {ai_loop_counter}", "AI"
                    )
                    if ai_loop_counter >= max_ai_loops:
                        raise Exception(
                            "AI appears stuck in a loop without simulator response."
                        )
            ...
            if args.p2_ai:
                ...
                if ai_req:
                    ...
                    debug_print(
                        f"P2 AI request ID: {ai_rqid}, last shown: {shown_rqid.get('p2')}",
                        "AI",
                    )
                    ...
                    if force_switch or shown_rqid.get("p2") != ai_rqid:
                        ...
                        observation = _create_agent_observation(
                            ai_req, battle, ui, current_turn
                        )
                        debug_print(
                            f"Created observation for LLM agent: turn {current_turn}",
                            "LLM_AGENT",
                        )
                        ...
                        if ai_req.get("wait") or ai_req.get("cant"):
                            debug_print(
                                "P2 AI has wait request - Pokemon can't move", "AI"
                            )
                        ...
                        try:
                            decision = _llm_agent_decision(observation, team_knowledge, raw_log=current_turn_log)
                            ...
                            if command:
                                ...
                                shown_rqid["p2"] = ai_rqid
                                ai_loop_counter = (
                                    0  # Reset counter on successful action
                                )
                                ai_error_count["p2"] = (
                                    0  # Reset error count on successful action
                                )
                                continue
                            else:
                                debug_print(
                                    "LLM agent failed to generate valid command", "AI"
                                )
                                ai_error_count["p2"] += 1
                        ...
                        if ai_error_count.get("p2", 0) >= max_ai_errors:
                            debug_print(
                                "Too many AI errors, using fallback logic", "AI"
                            )
                            ...
                            if (ai_req.get("forceSwitch") or [False])[0]:
                                ...
                                if switches:
                                    ...
                                    if ui and ui.enabled:
                                        ...
                                        print(note)
                                    debug_print(
                                        f"P2 AI fallback switch: {choice}", "AI"
                                    )
                                    shown_rqid["p2"] = ai_rqid
                                    ai_error_count["p2"] = 0
                                    continue
                                    ...
                                    all_fainted = all(
                                        poke.get("condition", "").endswith("fnt")
                                        for poke in ai_req.get("side", {}).get(
                                            "pokemon", []
                                        )
                                    )
                                    if all_fainted:
                                        debug_print(
                                            "P2 AI: All Pokemon fainted - game should end",
                                            "AI",
                                        )
                                        sim.send(">p2 forfeit")
                                        debug_print(
                                            "P2 AI: Sent forfeit due to no available Pokemon",
                                            "AI",
                                        )
                                    shown_rqid["p2"] = ai_rqid
                                    break
            ...
            if args.p2_ai and requests.get("p2"):
                ...
                if ai_loop_counter >= max_ai_loops:
                    debug_print(
                        "AI loop limit reached - simulator not responding, ending battle",
                        "AI",
                    )
                    if ui and ui.enabled:
            ...
            if player_req:
                ...
                if player_req.get("wait"):
                    ...
                    if player_rqid is None:
                        player_rqid = hash(
                            ...
                            + "|"
                            + str(player_req.get("wait"))
                        )

                    if shown_rqid.get(args.side) != player_rqid:
                        debug_print(
                            f"Player {args.side} has wait request - Pokemon can't move",
                            "WAIT",
                        )
                        ...
                        if additional_out:
                            debug_print(
                                f"Got additional output after wait: {len(additional_out)} lines",
                                "WAIT_RECOVERY",
                            )
                ...
                # Handle normal requests (active moves or forced switches)
                    ...
                    if shown_rqid.get(args.side) != player_rqid:
                        ...
                        try:
                            choice = _show_pokemon_showdown_menu(
                                player_req, battle, args.side, ui
                            )
